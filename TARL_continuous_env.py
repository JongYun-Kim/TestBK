...
class TAEnvDynaInEpisodeContinuous(Env):
    """
    ...
    def __init__(self, num_uav, num_task, is_moving, task_speed, is_ns_in_episode, change_point, acceleration):
        ...
        self.initial_task_speed = np.array(task_speed)
    ...
    def _take_into_grid(self):
        ...
        while changed:
            ...
            for i, x in enumerate(self.task_position.reshape(-1, 1)):
                if x > 1.0:
                    self.task_position.reshape(-1, 1)[i] = 2 - x  # 1-(x-1)==2-x
...
class TAEnvRandSpeedContObs(Env):
    """
    - Gym Env에 상속된 클래스 (step, render, reset)
    - 시나리오: 정해진 수의 UAV-들이 랜덤한 초기점에서 시작해서 공간에 랜덤으로 산포된 task-들의 위치를 모두 방문하는 시나리오.
    ...
    def __init__(self,
                 ...
                 num_task: int,
                 is_moving: bool,
                 task_speed_min: float,
                 task_speed_max: float,
                 rand_speed_in_episode: bool,
                 ):
        """
            :param num_uav: (int) the number of UAVs
            :param num_task: (int) the number of tasks
            :param is_moving: (bool) if the tasks move or not
            :param task_speed_min: (float) min speed
            :param task_speed_max: (float) max speed
            :param rand_speed_in_episode: (bool)
        """
        # Define variables constant during the run
        self.num_uav = int(num_uav)
        self.num_task = int(num_task)
        ...
        self.time_step = None
        self.is_moving = is_moving
        self.initial_task_speed = None  # np.array(task_speed)
        self.task_speed_min = float(task_speed_min)
        self.task_speed_max = float(task_speed_max)
        self.is_ns_in_episode = rand_speed_in_episode
        if abs(task_speed_max - task_speed_min) < 1e-10:  # task_speed_max == task_speed_min
            self.is_speed_fixed = True
        else:
            self.is_speed_fixed = False
    ...
    def _in_episode_ns(self):
        ...
        :return: None
        """
        # Change the speed of tasks
        self.task_speed += np.random.normal(loc=0.0, scale=0.005)
        # Clip the speed
        if self.task_speed < self.task_speed_min:
            self.task_speed = 2*self.task_speed_min - self.task_speed
            if self.task_speed > self.task_speed_max:
                self.task_speed = self.task_speed_min
            # self.task_speed = self.task_speed_min
        elif self.task_speed > self.task_speed_max:
            self.task_speed = 2*self.task_speed_max - self.task_speed
            if self.task_speed < self.task_speed_min:
                self.task_speed = self.task_speed_max
            # self.task_speed = self.task_speed_max
    ...
    def step(self, action: np.ndarray):
        ...
        # In-episode non-stationarity
        if self.is_ns_in_episode and self.is_moving and not(self.is_speed_fixed):
            self._in_episode_ns()
    ...
    def reset(self, do_over=False):
        ...
        if has_been_reset is not True or do_over is not True:  # 리셋 안해봤거나 반복 안하는 경우
            self.initial_state = self.observation_space.sample()
            self.initial_task_direction = self._init_task_direction()
            self.initial_task_speed = self.task_speed_min if self.is_speed_fixed else self._init_task_speed()
    ...
    def _init_task_speed(self):
        ...
        initial_speed = (1-alpha)*self.task_speed_min + alpha*self.task_speed_max
