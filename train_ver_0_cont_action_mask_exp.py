...
# from TARL_continuous_env import TAEnvDynaInEpisodeContinuous
from TARL_cont_action_mask_env import TAEnvDynaInEpContActionMasked
from fc_action_masking_model import FCActionMaskModelJY
...
def trial_dir_name_creator(trial):
    ...
    now = datetime.now()
    time_str = f"action_mask_{now.year}-{now.month}-{now.day}_{now.hour}-{now.minute}-{now.second}"
...
if __name__ == "__main__":
    ...
    # register_env("NSenv3", lambda _: TAEnvDynaInEpisodeContinuous(num_uav, num_task, False, 0, False, 1, 0))
    register_env("NSenv3_mask", lambda _: TAEnvDynaInEpContActionMasked(num_uav, num_task, False, 0, False, 1, 0))
    target_env = "NSenv3_mask"

    # Register your model
    ModelCatalog.register_custom_model(
        "action_mask_model", FCActionMaskModelJY
    )
    ...
    # save_name = "cont_env_tests"
    # save_name = "action_mask_test_dir"
