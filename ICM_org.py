...
# @override
class ReactiveExplorationOrg(Curiosity):
    """
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 re_net_config: Optional[ModelConfigDict] = None,
                 **kwargs):
        ...
        super().__init__(action_space,
                         ...
                         **kwargs)
        for my_i in range(2):
            print("The Super function has been just implemented")
        ...
        if feature_net_config is None:
            ...
            print(self.policy_config["model"].copy())
        ...
        print([2 * self.feature_dim] + list(self.inverse_net_hiddens) + [self.action_dim])
        print(f"inverse_net_hiddens = {self.inverse_net_hiddens}")
        print("inverse_net = (shown below)")
        print(self._curiosity_inverse_fcnet)
        ...
        print("forward_net = (shown below)")
        print(self._curiosity_forward_fcnet)
    ...
    def _postprocess_torch(self, policy, sample_batch):
        ...
        # print(f"length of obs in the sample batch = {len(sample_batch[SampleBatch.OBS])}")
        # print(f"length of new_obs in the sample batch = {len(sample_batch[SampleBatch.NEXT_OBS])}")
        # print(f"sample_batch[SampleBatch.OBS=obs] = {sample_batch[SampleBatch.OBS]}")
        # print(f"sample_batch[SampleBatch.NEXT_OBS=new_obs] = {sample_batch[SampleBatch.NEXT_OBS]}")
        # print(f"sample_batch[eps_id] ={sample_batch[SampleBatch.EPS_ID]}")
        # print(f"sample_batch[actions] = {sample_batch[SampleBatch.ACTIONS]}")
        # print(f"sample_batch[action_dist_inputs] = {sample_batch[SampleBatch.ACTION_DIST_INPUTS]}")
        # print(f"sample_batch[action_logp] = {sample_batch[SampleBatch.ACTION_LOGP]}")
        # print(f"SampleBatch = {SampleBatch}")
        # print(f"SampleBatch.OBS = {SampleBatch.OBS}")
        # print(f"SampleBatch.NEXT.OBS = {SampleBatch.NEXT_OBS}")
        phi, next_phi = torch.chunk(phis, 2)
        actions_tensor = torch.from_numpy(
            sample_batch[SampleBatch.ACTIONS]).long().to(policy.device)
        print(f"action_tensor = {actions_tensor}")
        print(f"SIZE of action_tensor = {actions_tensor.shape}")
        print(f"sample_batch[SampleBatch.ACTIONS] = {sample_batch[SampleBatch.ACTIONS]}")
        print(f"SIZE of sample_batch[SampleBatch.ACTIONS] = {sample_batch[SampleBatch.ACTIONS].size}")
        print(f"sample_barch[SampleBatch.REWARDS] = {sample_batch[SampleBatch.REWARDS]}")
        print(f"SIZE of sample_barch[SampleBatch.REWARDS] = {sample_batch[SampleBatch.REWARDS].size}")
        # print(f"phi = {phi}")
        print(f"SIZE of phi = {phi.shape}")
        # print(f"phi is tensor = {torch.is_tensor(phi)}")
        ...
        print(f"forward_fcnet input size = {torch.cat([phi, one_hot(actions_tensor, self.action_space).float()], dim=-1).shape}")
        print(f"re_net input size = {torch.cat([torch.from_numpy(sample_batch[SampleBatch.OBS]), one_hot(actions_tensor, self.action_space).float()], dim=-1)}")
        ...
        # print(f"shape of forward_l2_norm_squared size = {forward_l2_norm_sqared.shape}")
        # print(f"forward_l2_norm_sqared is tensor = {torch.is_tensor(forward_l2_norm_sqared)}")
        print(f"forward_loss = {forward_loss} == average intrinsic_reward / eta")
        # print(f"forward_loss size = {forward_loss.shape}")
        # print(f"forward_loss is tensor = {torch.is_tensor(forward_loss)}")
