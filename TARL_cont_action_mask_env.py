...
class TAEnvDynaInEpContActionMasked(Env):
    """
    ...
    def __init__(self, num_uav, num_task, is_moving, task_speed, is_ns_in_episode, change_point, acceleration):
        ...
        self._my_logit_size = self.num_uav * (self.num_task+1)
        ...
        self.state = None
        self.valid_actions = None  # == action mask == obs["action_mask"]
        self.state_w_mask = None  # == {"action_mask": self.valid_actions, "observation": self.state}
    ...
    def _update_action_mask(self, action: np.ndarray):
        # [안1] self.task_completion에서 끝난 task를 가지고 action mask update
        # [안2] action을 가져와서 action_mask를 update
        for task_number in action:  # [안2]를 가지고 함
            if task_number == 0:  # 움직이지 않는 경우는 masking 하지 않음
                continue
            # UAV별 logit mask를 각 행에 1횡-어레이로 구성하고.. 모든 실행된 task는 모든 uav에 대해 mask out
            self.valid_actions.reshape(self.num_uav, -1)[:, task_number] = 0.0
    ...
    def step(self, action: np.ndarray):
        ...
        # Update action mask
        self._update_action_mask(action)  # It must be done before task completion update!!
        ...
        return self.state_w_mask.copy(), reward, done, info
